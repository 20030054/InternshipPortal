...
import { approveWaiver, WaiverNotInProgressError } from "@/server/waivers/service";
import {
...
/** `waiver.approve_final` (DEAN): fires row 24, WAIVER_COUNTERSIGNED ->
 * WAIVER_GRANTED. Only reachable once the HoD has already
 * countersigned — the Dean cannot skip ahead of the HoD stage. */
export async function POST(
  ...
  try {
    ...
    const waiver = await approveWaiver({
      waiverId: id,
